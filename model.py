...
class Model(SoftmaxClassifier):
    ...
    def get_logits(self, inputs):
        ...
        net = ti.layers.conv2d_densefire_block(
            ...
            n_layers=16, compression=0.85, activation=tf.nn.elu, padding="same",
        ...
        # dense 2
        net = ti.layers.conv2d_densefire_block(
            net, bottleneck=48, growth_rate_1x1=12, growth_rate_3x3=12,
            n_layers=10, compression=None, activation=tf.nn.elu, padding="same",
            dropout = dict(rate = 0.2), batch_norm = dict(training = inputs.training)
        ); print("Dense Fire Block (48, 6, 6, 16): {}".format(net))
